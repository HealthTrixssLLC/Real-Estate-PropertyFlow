...
async function uploadItem(item: PendingUpload): Promise<boolean> {
  ...
  try {
    // Bare RN can fetch a local `file://` URI directly and yield a Blob —
    // no base64 round-trip needed (we previously used expo-file-system's
    // legacy readAsStringAsync for this on Expo SDK 54).
